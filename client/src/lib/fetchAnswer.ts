...
const mockResponses: FetchAnswerResponse[] = [
  ...
  {
    answer: "Sarah discussed planning a weekend beach trip to Malibu. She was excited about the ocean views and mentioned wanting to go surfing for the first time.",
    snippet: [
      {
        speaker: "fan",
...
export async function fetchAnswer({ fan, model = "default_model", question }: FetchAnswerRequest): Promise<FetchAnswerResponse> {
  ...
  //     model, 
  //     question,
  //     // Prompt template: "@{fan} for {model} → {question}. Return a concise answer and the exact conversation snippet that supports it."
  //   })
  // });
  // 
  // if (!response.ok) {
